  ...
  <div :data-field-name="name" class="mobile-form-field">
    ...
    >
      ...
      <template v-slot:selected-item="scope">
        <div class="row items-center no-wrap">
          <div class="col">
            <div>{{ scope.opt.label || 'Address' }}</div>
            <div class="text-caption text-grey-7">{{ scope.opt.oneLine }}</div>
          </div>
        </div>
      </template>

      <template v-slot:after-options v-if="allowCreate && vendorId">
      ...
      <template v-slot:no-option>
        <q-item v-if="vendorId">
          <q-item-section>
            <q-item-label>No addresses for this vendor</q-item-label>
          </q-item-section>
        </q-item>
        <q-item clickable @click="openAddAddressDialog" v-if="allowCreate && vendorId">
          <q-item-section avatar>
            <q-icon name="add_circle" color="primary" />
          </q-item-section>
          <q-item-section>
        ...
        <q-item v-if="!vendorId">
          <q-item-section>
            <q-item-label class="text-grey-6">Select a vendor first</q-item-label>
          </q-item-section>
        </q-item>
      </template>
    ...
    <q-dialog v-model="showAddDialog" persistent>
      <q-card style="min-width: 350px; max-width: 90vw;">
        <q-card-section>
          <div class="text-h6">Add Vendor Address</div>
        </q-card-section>
...
// Auto-select primary address when vendor changes
